...
- (void)configure{
    ...
    _titleFontSize = 18;
    _titleColor = [UIColor xy_colorWithHex:0x222222];
    
    _contentFontSize = 15;
    _contentColor = [UIColor xy_colorWithHex:0x444444];
    
    _btnBackgroundColor = [UIColor xy_colorWithHex:0x23a1fa];
    _btnTitleColor = [UIColor whiteColor];
    _btnTitleFontSize = 15;
//    _btnBorderColor = [UIColor XY_colorWithHex:0x1DBDB8];
...
- (void)layoutUI{
    ...
    [self.cancelBtn xy_shadowWithCornerRadius:twoBtnHeight/2 borderWidth:1.2 borderColor:_btnBackgroundColor shadowColor:[UIColor xy_colorWithHex:0x00C3F2] shadowOpacity:0.4 shadowOffset:CGSizeMake(0, 2.0) roundedRect:CGRectMake(3, 2, twoBtnWidth-6, twoBtnHeight) cornerRadii:CGSizeMake(twoBtnHeight/2, twoBtnHeight/2) rectCorner:UIRectCornerAllCorners];
    ...
    [self.destructiveBtn xy_shadowWithCornerRadius:twoBtnHeight/2 borderWidth:0 borderColor:nil shadowColor:[UIColor xy_colorWithHex:0x00C3F2] shadowOpacity:0.5 shadowOffset:CGSizeMake(0, 2.5) roundedRect:CGRectMake(3, 2, twoBtnWidth-6, twoBtnHeight) cornerRadii:CGSizeMake(twoBtnHeight/2, twoBtnHeight/2) rectCorner:UIRectCornerAllCorners];
    ...
    [self.sureBtn xy_shadowWithCornerRadius:twoBtnHeight/2 borderWidth:0 borderColor:nil shadowColor:[UIColor xy_colorWithHex:0x00C3F2] shadowOpacity:0.5 shadowOffset:CGSizeMake(0, 2.5) roundedRect:CGRectMake(3, 2, sureWidth-6, twoBtnHeight) cornerRadii:CGSizeMake(sureWidth, twoBtnHeight/2) rectCorner:UIRectCornerAllCorners];
...
#pragma mark - Public
+ (XYLancooAlert *)lancooAlertWithTitle:(NSString *)title msg:(NSString *)msg sureTitle:(NSString *)sureTitle sureBlock:(void (^)(void))sureBlock{
    XYLancooAlert *alertView = [[XYLancooAlert alloc] init];
...
+ (XYLancooAlert *)lancooAlertWithTitle:(NSString *)title msg:(NSString *)msg cancelTitle:(NSString *)cancelTitle destructiveTitle:(NSString *)destructiveTitle cancelBlock:(void (^)(void))cancelBlock destructiveBlock:(void (^)(void))destructiveBlock{
    XYLancooAlert *alertView = [[XYLancooAlert alloc] init];
...
+ (XYLancooAlert *)lancooAlertWithTitle:(NSString *)title msgAttr:(NSAttributedString *)msgAttr sureTitle:(NSString *)sureTitle sureBlock:(void (^)(void))sureBlock{
    XYLancooAlert *alertView = [[XYLancooAlert alloc] init];
    alertView.destructiveBlock = sureBlock;
...
+ (XYLancooAlert *)lancooAlertWithTitle:(NSString *)title msgAttr:(NSAttributedString *)msgAttr cancelTitle:(NSString *)cancelTitle destructiveTitle:(NSString *)destructiveTitle cancelBlock:(void (^)(void))cancelBlock destructiveBlock:(void (^)(void))destructiveBlock{
    XYLancooAlert *alertView = [[XYLancooAlert alloc] init];
    alertView.cancelBlock = cancelBlock;
    alertView.destructiveBlock = destructiveBlock;
    alertView.contentL.hidden = YES;
...
+ (XYLancooAlert *)lancooAlertGifViewWithGifName:(NSString *)gifName msg:(NSString *)msg duration:(NSInteger)duration{
    ...
    [alertView xy_clipLayerWithRadius:5 width:0 color:nil];
    UIImageView *gifImg = [[UIImageView alloc] initWithImage:[UIImage xy_animatedImageNamed:gifName atDir:nil duration:duration atBundle:XYAlert.alertBundle]];
    ...
    [gifImg xy_clipLayerWithRadius:50 width:0 color:nil];
...
+ (void)dismiss{
    ...
    for (UIView *view in rootWindow.subviews) {
        if ([view isKindOfClass:[XYLancooAlert class]]) {
            [(XYLancooAlert *)view hide];
        }
    }
}
- (void)show{
